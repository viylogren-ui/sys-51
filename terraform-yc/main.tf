...
resource "yandex_compute_instance" "web_db" {
  name        = "web-db" #Имя ВМ в облачной консоли
  hostname    = "web-db" #формирует FDQN имя хоста, без hostname будет сгенрировано случаное имя.
  platform_id = "standard-v3"
  zone        = "ru-central1-b" #зона ВМ должна совпадать с зоной subnet!!!
...
resource "local_file" "inventory" {
  ...
  [database]
  ${yandex_compute_instance.web_db.network_interface.0.ip_address}
